...
func import_model(file_path: String, options: Dictionary = {}) -> ModelData:
	...
	Debug.call_deferred("log", "\n=== OBJ DEBUG INFO ===")
	Debug.call_deferred("log", "File: ", file_path)
	Debug.call_deferred("log", "Total lines parsed: ", line_number)
	Debug.call_deferred("log", "Raw vertex lines found: ", debug_info.raw_vertices.size())
	Debug.call_deferred("log", "Raw face lines found: ", debug_info.raw_faces.size())
	Debug.call_deferred("log", "Vertices parsed: ", vertex_count)
	Debug.call_deferred("log", "Normals parsed: ", normal_count)
	Debug.call_deferred("log", "UVs parsed: ", uv_count)
	Debug.call_deferred("log", "Faces parsed: ", face_count)
	Debug.call_deferred("log", "Triangle count: ", triangle_count)
	...
	if ngon_count > 0:
		...
		Debug.log("OBJ Import Warning: ", ngon_count, " n-gons detected in file")
		for ngon in ngon_faces:
			Debug.log("  Line ", ngon.line, ": ", str(ngon.vertex_count) + " vertices - " + ngon.content)
...
func create_wireframe_mesh(model_data: ModelData) -> MeshInstance3D:
	...
	var read_check = ErrorHandler.check_file_read(file_path)
	if not read_check.success:
		# Dont show error popup for wireframe
		Debug.log("OBJFormat: Cannot read file for wireframe, using default: " + file_path)
		return super.create_wireframe_mesh(model_data)
