...
var _max_concurrent_tasks: int = 4 # Should never exceed 1 without increasing the pool in main
...
func _init(format_registry = null, max_concurrent_tasks: int = 4):
	...
	_mutex = Mutex.new()
	_initialize_workers()
	Debug.log("(ThreadPool) Initialize Workers")
...
func _initialize_workers():
	for i in range(_max_concurrent_tasks):
		...
		Debug.log("(ThreadPool) Workers Initialized"," ", worker)
	
	#for worker in _workers: # Redundant // Resources already allocated in ConversionWorker._init
...
func queue_conversion(source_path: String, target_path: String, options: Dictionary = {}) -> int:
	...
	Debug.log("(ThreadPool) Conversion Queued")
...
func cancel_task(task_id: int) -> bool:
	Debug.log("(ThreadPool) Canceling Task")
	_mutex.lock()
	...
	_mutex.unlock()
	Debug.log("(ThreadPool) Task Canceled")
	return false
...
func cancel_all_tasks() -> void:
	...
	for task_id in active_ids:
		cancel_task(task_id)
	Debug.log("(ThreadPool) Canceling All Tasks")
...
func _on_worker_conversion_started(task_id: int, source_path: String, target_path: String) -> void:
	Debug.log("(ThreadPool) Conversion Starting")
...
func _on_worker_conversion_progress(task_id: int, progress: float) -> void:
	Debug.log("(ThreadPool) Conversion Progress: ", progress)
...
func _on_worker_conversion_completed(task_id: int, result: Dictionary) -> void:
	...
	Debug.log("(ThreadPool) Conversion Completed: ", result)
...
func _on_worker_conversion_error(task_id: int, error_message: String) -> void:
	...
	Debug.log("(ThreadPool) Conversion Error", task_id, error_message)
...
func _check_all_completed() -> void:
	...
	if pending_count == 0 and active_count == 0:
		Debug.log("(ThreadPool) Tasks Completed")
	...
	match criteria:
		...
		"size":
			_task_queue.sort_custom(func(a, b): 
				# Add Read/Write Error Checks here
